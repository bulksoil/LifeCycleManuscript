...
data.2016 <- readRDS("~/Google Drive/RMB/Analyses/LifeCycle/Data/arb_2016_data.rds")
...
arb2016.pc$axes %>% 
  ...
  nest() %>% 
  mutate(model = map(data, ~lm(MDS2 ~ ns(Day_After_Germination, 2) * Genotype, .)),
         null_mod = map(data, ~lm(MDS2 ~ ns(Day_After_Germination, 2) + Genotype, .))) %>% 
  unnest(map2(model, null_mod, anova)) %>% 
  na.omit()
  
axis.model <- arb2016.pc$axes %>% 
  mutate(Compartment = fct_relevel(Compartment, "Bulk Soil", "Rhizosphere", "Rhizoplane", "Endosphere")) %>% 
  group_by(Compartment) %>%
  filter(Compartment != "Bulk Soil") %>% 
  nest() %>% 
...
axis.model.soil <- arb2016.pc$axes %>% 
  ...
  filter(Compartment == "Bulk Soil") %>%
  nest() %>% 
  mutate(model = map(data, ~lm(MDS2 ~ ns(Day_After_Germination, 2), .))) %>% 
  unnest(map(model, ~tidy(.)))

kit_estimates <- axis.model %>% filter(term == "ns(Day_After_Germination, 2)1") %>% 
  select(Compartment, estimate, std.error) %>% 
  mutate(Genotype = "Kitaake")

axis.model %>% 
  filter(grepl("1:", term)) %>% 
  mutate(Genotype = gsub(".*Genotype", "", term)) %>% 
  select(Compartment, Genotype, estimate, std.error) %>% 
  bind_rows(kit_estimates) %>% 
  rename(geno_change = estimate) %>% 
  inner_join(kit_estimates %>% select(Compartment, estimate), by = "Compartment") %>% 
  mutate(new_estimate = ifelse(Genotype != "Kitaake", estimate + geno_change, estimate)) %>% 
  bind_rows(axis.model.soil %>% filter(term == "ns(Day_After_Germination, 2)1") %>% rename(new_estimate = estimate) %>%  mutate(Genotype = "Soil")) %>% 
