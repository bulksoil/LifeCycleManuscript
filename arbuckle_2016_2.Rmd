...
data.2016 <- data.2016 %>% inner_join(runs, by = "SampleID")
...
```{r}
m206v401permanovaPRE <- long_adonis(data.2016 %>% ungroup() %>% filter(Genotype == "M401" | Genotype == "M206") %>% filter(Day_After_Germination <= 63), 
                                    formula = "Compartment * Day_After_Germination + Genotype")
m206v401permanovaPOST <- long_adonis(data.2016 %>% ungroup() %>% filter(Genotype == "M206" | Genotype == "M401") %>% filter(Day_After_Germination > 63), 
                                    formula = "Compartment * Day_After_Germination + Genotype")
```
...
```{r}
m <- arb2016.pc$axes %>% 
  group_by(half = ifelse(Day_After_Germination <= 77, "pre77", "post77"), Compartment) %>% 
