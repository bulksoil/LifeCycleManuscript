...
map <- lc_data %>% 
  group_by(Compartment, Season, SampleID, Age, State, Site, Depth) %>% 
  filter(value > 0) %>% 
...
age.pc <- lc.pc$axes %>% 
  ggplot(aes(MDS1, MDS3, color = Age)) +
  geom_point(alpha = 0.8) +
  theme_minimal() +
...
whole_permanova <- long_adonis(lc_data, formula = "Site * Compartment * Age")
ark_permanova <- long_adonis(lc_data %>% filter(Site == "Jonesboro"), formula = "Compartment * Age * Cultivar")
arb_permanova <- long_adonis(lc_data %>% filter(Site == "Arbuckle"), formula = "Compartment * Age * Season")
```
